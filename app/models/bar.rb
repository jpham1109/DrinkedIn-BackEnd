...
class Bar < ApplicationRecord
  ...
  def place_id
    gm_place_search_url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=40.7128,-74.0060&radius=15000&type=bar&keyword=#{name}&key=#{ENV['GM_KEY']}"
  ...
  def address
    gm_address_search_url = "https://maps.googleapis.com/maps/api/place/details/json?place_id=#{place_id}&fields=formatted_address&key=#{ENV['GM_KEY']}"
  ...
  def website
    gm_website_search_url = "https://maps.googleapis.com/maps/api/place/details/json?place_id=#{place_id}&fields=website&key=#{ENV['GM_KEY']}"
  ...
  def photoref_array
    gm_detail_url = "https://maps.googleapis.com/maps/api/place/details/json?place_id=#{place_id}&fields=photos&key=#{ENV['GM_KEY']}"
  ...
  def reviews
    gm_detail_url = "https://maps.googleapis.com/maps/api/place/details/json?place_id=#{place_id}&fields=reviews,rating,user_ratings_total&key=#{ENV['GM_KEY']}"
    response = Faraday.get gm_detail_url
    res = JSON.parse(response.body)
    res['result']['reviews']
  end
